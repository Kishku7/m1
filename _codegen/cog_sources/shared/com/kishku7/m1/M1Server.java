...
public final class M1Server {
    ...
    private static final int MAX_LINE = 1 << 20; // D19: 1 MiB cap on an inbound line -- no unbounded wire line into heap
    ...
    // D19: BufferedReader.readLine buffers a whole line with NO cap -- a hostile/buggy local client
    // could stream an unbounded line and OOM the client heap. Read char-by-char with a hard cap; an
    // over-cap line is drained to the newline and dropped (returned "" -> treated as empty). An idle
    // timeout BEFORE any char propagates (so the caller pushes async events); a mid-line timeout keeps
    // waiting for the rest of the line. Returns null at end of stream (like readLine).
