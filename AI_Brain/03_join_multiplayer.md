...
## Before you connect
Load `ask_the_user.md` and confirm:
...
- That the user actually wants to join THIS server and is allowed to be there. If it is someone else's
  server, also read `safety_etiquette.md` before doing anything in-world.
...
3. Expect a "Connecting..." screen. Then verify: `where`. `pos=(...)` = you are in. A failure screen
   ("Can't connect", timeout, version mismatch) means STOP and report it -- see `recovery.md`; do not
   spam retry.

## First moments on a shared server
- `where` + `scan`, give a short situation report, and ASK the user what they want before acting.
- Follow the server's rules and `safety_etiquette.md`: no griefing, stealing, or PvP unless the user
